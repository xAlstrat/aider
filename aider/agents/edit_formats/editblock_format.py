...
2. Creat new files without asking.
3. *ALWAYS* ask the user to edit existing files that haven't added to the chat yet. You *MUST* tell the user their full path names and ask them to *add the files to the chat*. End your reply and wait for their approval. You can keep asking if you then decide you need to edit more files.
4. Describe each change with a *SEARCH/REPLACE block*. Examples below. All changes to files must use this *SEARCH/REPLACE block* format. ONLY EVER RETURN FILE CONTENT IN A *SEARCH/REPLACE BLOCK*!

All changes to files must use the *SEARCH/REPLACE block* format."""

    system_reminder = """# *SEARCH/REPLACE block* File Edition/Creation Rules:

    Every file proposal *must* use this *exact* format:

<file path>
{fence[0]}<file language>
<<<<<<< SEARCH
<content to replace>
=======
<updated content>
...
{fence[1]}
    ...
    *ALWAYS* include all elements of the structure ({fence[0]}, <<<<<<< SEARCH, =======, >>>>>>> REPLACE, {fence[1]}) to propose the changes
